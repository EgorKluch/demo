...
const FILES_TO_PARSE = 'src/**/!(*.test).+(js|ts|tsx)';
...
for (const locale of locales) {
    ...
    const translationFileName = `assets/intl/${locale}.json`;
    try {
    ...
    try {
        ...
        for (const message of messages) {
            for (const locale of locales) {
                ...
                const newMsg = locale === 'ru' ? message.defaultMessage : '';
...
memoryTask.then(files => {
    ...
    extractTask.then(() => {
        ...
        mkdir('-p', 'assets/intl'); // eslint-disable-line

        let localeTaskDone;
        let translationFileName;

        for (const locale of locales) {
            translationFileName = `assets/intl/${locale}.json`;
            localeTaskDone = task(
